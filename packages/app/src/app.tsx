...
import { I18nProvider } from "@diveeoi/ui/context"
import { DialogProvider } from "@diveeoi/ui/context/dialog"
import { FileComponentProvider } from "@diveeoi/ui/context/file"
import { MarkedProvider } from "@diveeoi/ui/context/marked"
import { File } from "@diveeoi/ui/file"
import { Font } from "@diveeoi/ui/font"
import { Splash } from "@diveeoi/ui/logo"
import { ThemeProvider, useTheme } from "@diveeoi/ui/theme/context"
import type { DesktopTheme } from "@diveeoi/ui/theme/types"
...
function _ExternalThemeLoader() {
  ...
  onMount(async () => {
    ...
    if (!base) return
    try {
      const res = await fetch(`${base}/_opencode/themes`)
      if (!res.ok) return
      const themes = (await res.json()) as Record<string, unknown>
      for (const [, data] of Object.entries(themes)) {
        if (data && typeof data === "object" && "id" in data && "name" in data && "light" in data && "dark" in data) {
          theme.registerTheme(data as DesktopTheme)
        }
      }
    } catch {}
  })

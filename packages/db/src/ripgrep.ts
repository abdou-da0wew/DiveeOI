...
function parseMatchLine(line: string): RawMatchData | undefined {
  try {
    const json = JSON.parse(line)
    if (typeof json !== "object" || json === null) return undefined
    if (json.type === "begin" || json.type === "end") return undefined
    if (json.type !== "match") return undefined
    const data = json.data
    if (!data || typeof data !== "object") return undefined
    const pathText = typeof data.path?.text === "string" ? data.path.text.replace(/^\.[\\/]/, "") : ""
    const submatches: SubMatch[] = (data.submatches || []).slice(0, MAX_SUBMATCHES)
    return {
      path: { text: pathText },
      lines: { text: data.lines?.text ?? "" },
      line_number: data.line_number ?? 0,
      absolute_offset: data.absolute_offset ?? 0,
      submatches,
    } as unknown as RawMatchData
  } catch {
    return undefined
  }
}

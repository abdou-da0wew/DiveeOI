...
import { EventSequenceTable } from "@diveeoi/db/event/sql"
...
const startWorkspaceSyncingWithFlag = (projectID: ProjectV2.ID, experimentalWorkspaces: boolean) =>
  Effect.runPromise(
    Workspace.use.startWorkspaceSyncing(projectID).pipe(
      Effect.provide(workspaceLayer(experimentalWorkspaces) as unknown as Layer.Layer<Workspace.Service, unknown>),
    ),
  )

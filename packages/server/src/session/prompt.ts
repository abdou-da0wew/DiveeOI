...
import { LLM } from "./llm"
import { Shell } from "@diveeoi/db/shell"
...
import { AgentAttachment, FileAttachment, Prompt, Source } from "@diveeoi/db/session/prompt"
...
function applyTokenWindow(msgs: SessionV1.WithParts[], maxTokens: number): SessionV1.WithParts[] {
  ...
  let compactionSummaryIdx = -1
  for (let i = msgs.length - 1; i >= 0; i--) {
    const msg = msgs[i]
    if (msg.info.role === "assistant" && msg.info.summary && msg.parts.some(p => p.type === "compaction")) {
      compactionSummaryIdx = i
      break
    }
  }
  let estimated = 0
  let cutIdx = 0
  for (let i = msgs.length - 1; i >= 0; i--) {
    estimated += estimateMessageTokens(msgs[i])
    if (estimated > maxTokens) {
      cutIdx = i + 1
      break
    }
  }
  if (cutIdx >= msgs.length) return msgs.length > 0 ? [msgs[msgs.length - 1]] : []
  if (compactionSummaryIdx >= 0 && compactionSummaryIdx < cutIdx) cutIdx = compactionSummaryIdx
  return cutIdx > 0 ? msgs.slice(cutIdx) : msgs
}
    ...
    const runLoop = Effect.fn("SessionPrompt.run")(
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const cfg = yield* config.get()
          msgs = applyTokenWindow(msgs, overflow.usable({ cfg, model }))
  ...
  layer.pipe(
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        SessionMemoryIntegration.defaultLayer,
      ),
    ),
  ),
)

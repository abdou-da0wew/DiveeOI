...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@diveeoi/plugin"
...
export class Service extends Context.Service<Service, Interface>()("@diveeoi/ToolRegistry") {}
  ...
  Effect.gen(function* () {
    ...
    const resolveLazy = yield* Effect.cached(
      Effect.gen(function* () {
        ...
        return yield* Effect.all(s.lazy.map((l: { init: Effect.Effect<Tool.Def> }) => l.init))
      }),
    )
...
export const defaultLayer = Layer.suspend(() =>
  layer
    ...
    .pipe(
      ...
      // The memory graph only builds when the feature is enabled; the
      // integration and scheduler layers degrade to no-op stubs on their own.
      Layer.provide(Features.flags.memory ? Memory.defaultLayer : Layer.empty),

...
- **Schema**: Drizzle schema lives in `packages/db/src/database/schema.gen.ts` (auto-generated from `schema.sql.ts`).
- **Migrations**: database migrations live in `packages/db/src/database/migration/`.

## Development server

- Server starts from `packages/server/src/main.ts` on port 4097 (config via `PORT` env var).
- Run: `bun run packages/server/src/main.ts` (or `bun dev` in `packages/server` if a dev script is configured).
